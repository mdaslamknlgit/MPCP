...
{
    public partial class RefineryPremisePinvConstraint
    {
        [Parameter]
        ...
        public async Task OnReadHandlerAsync(GridReadEventArgs args)
        {
            if (!IsReady)
            {
                args.Data = Enumerable.Empty<Model.PinvConstraintModel>();
                args.Total = 0;
                return;
            }

            var result = await BuildGridResultAsync(args.Request, PinvConstraintModelData, x => x.ConstraintTag);
