...
namespace MPC.PlanSched.UI.Pages.HistoricalPlans
{
    ...
    public partial class HistoricalPlansRP
    {
        [Parameter]
        ...
        private async Task GetRegionsAsync()
        {
            ...
            try
            {
                _regionsList = await UtilityUI.GetRegionListAsync(ActiveUser, ConfigurationUI.domainNamespaceTypeRP, Client, LocalTimeZoneName, SessionService.GetCorrelationId(), true);
        ...
        private void ViewPlan(BusinessCase businessCaseObj)
        {
            ...
            NavigateToProductRP(SelectedRole, businessCaseId, true);
        }
        private async Task ExcelDownloadAsync(BusinessCase businessCaseObj)
        {
            try
            {
                ...
                LockLoading();
                var regionObj = CreateRegion(businessCaseObj);
                var base64String = await _excelCommon.GetExcelBase64ByRegion(regionObj, ApplicationArea.regionalplanning);
        ...
        private RegionModel CreateRegion(BusinessCase businessCaseObj)
        {
            ...
            var regionObj = regionModelObj;
            regionObj.BusinessCase = businessCaseObj;
            regionObj.IsHistoricalPlan = true;
            return regionObj;
        }
